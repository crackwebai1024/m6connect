<template>
    <div class="d-flex blue darken-1 white--text pl-3 pr-2 py-2">
        <div v-if="indexState==0" class="cursor-hover white--text" @click="preview_screen(recorIdle)" style="font-size: 17px;">{{ recorIdle.title }}</div>
        <div v-else class="cursor-hover white--text" @click="unshift_data_to_active(recorIdle)" style="font-size: 17px;">{{ recorIdle.title }}</div>
