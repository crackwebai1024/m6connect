            ...
            >
              <div v-if="searchBudgetCategoriesArray.length">
                <v-list three-line>
                  ...
                  >
                    <v-list-tile-content>
                      <v-list-tile-sub-title>{{ catItem.name }}</v-list-tile-sub-title>
                    </v-list-tile-content>
                  </v-list-tile>
                </v-list>
              </div>
...
export default {
  ...
  computed: {
    ...
    searchBudgetCategoriesArray() {
      ...
      this.costCodesAllArray.map(cc => {
        if (cc.name.toLowerCase().includes(this.commitment.costCodeText.toLowerCase())) {
          result.push(cc)
        }
      })
      return result
  ...
  firestore() {
    return {
      ...
      commitment: this.projectRef.collection('commitments').doc(this.commitmentId + ''),
  ...
  watch: {
    ...
    project() {
      ...
      const folder = this.project.files[1].children[9].children.find(dir => dir.path === path)
      if (folder) {
  ...
  methods: {
    ...
    setCostCodeL2(item, itemL2) {
      if (item.code) {
        this.commitment.costCodeText = itemL2.code + ' - ' + item.name + ' - ' + itemL2.name
      } else {
    ...
    setCostCodeL3(item, itemL2, itemL3) {
      if (item.code) {
        this.commitment.costCodeText = itemL3.code + ' - ' + item.name + ' - ' + itemL2.name + ' - ' + itemL3.name
      } else {
        this.commitment.costCodeText = item.name + ' - ' + itemL2.name + ' - ' + itemL3.name
      }
      this.commitment.budgetCategory = item.name
      this.commitment.costCode = item.name
      this.commitment.costCodeL2 = itemL2.name
    ...
    save() {
      ...
      if (this.lineItem.startDateText) {
        const startDate = DateTime.fromFormat(this.lineItem.startDateText, 'MMddyyyy')
      ...
      if (this.lineItem.deliveryDateText) {
        const deliveryDate = DateTime.fromFormat(this.lineItem.deliveryDateText, 'MMddyyyy')
      ...
      if (this.lineItem.completionDateText) {
        const completionDate = DateTime.fromFormat(this.lineItem.completionDateText, 'MMddyyyy')
      ...
      if (formIsValid) {
        ...
        if (this.lineItem.change) {
          const changeArr = []
          this.lineItem.change.forEach(change => {
            const changeRef = this.projectRef.collection('changes').doc(change.id)
        ...
        const path = this.project.files[1].children[9].path + '/' + this.commitment.id
        ...
        this.projectRef.collection('commitments').doc(this.commitmentId + '').update(this.commitment)
        this.projectRef.collection('commitments').doc(this.commitmentId + '').collection('line_items').doc(this.lineItemId + '').update(this.lineItem)
