<template>
    <v-container fluid grid-list-md class="ma-0 pa-0 fill-height">
      <v-layout>
        <v-flex xs12>
          <v-card flat>
            <v-card-text>
                  ...
                  <div class="tableWrap">
                    <v-card>
                      <v-card-title>
                          ...
                          >
                            ...
                            <v-flex>
                              ...
                              >
                                ...
                                <template slot="no-options">{{ $t('milestoneDashboard.searchCampus') }}...</template>
                              </v-select>
                            </v-flex>
                            ...
                            <v-flex>
                              ...
                              >
                                ...
                                <template slot="no-options">{{ $t('ReportsGeneral.typeSearchUsers') }}</template>
                              </v-select>
                            </v-flex>
                        ...
                        <v-btn :loading="loading" color="blue" dark @click="filterAll"
                          >{{ $t('milestoneDashboard.generate') }}</v-btn
                        >
                        <v-btn :loading="loadingExport" color="green" dark @click="exportReport" :disabled="!canExport"
                          >{{ $t('milestoneDashboard.exportReport') }}</v-btn
                        >
                        <v-spacer></v-spacer>
                        <v-text-field
                          v-model="search"
                        ...
                        >
                          <template v-slot:headers="props">
                            <tr class="first">
                              ...
                              v-for="(h, i) in props.headers">{{ h.text }}</th>
                              <th width="200" colspan="3" v-for="(milestone, index) in milestones" :key="index" :style="{ 'background-color': milestone.color ? milestone.color.hex : '#EFEFEF'}">{{ milestone.name ? milestone.name : milestone }}</th>
                            </tr>
                            <tr class="second">
                              <th width="100" v-for="n in (milestones.length*3)" :style="{ 'background-color': getColor(n) }">{{ (n%3 == 1) ? 'Original' : n%3 == 2 ? 'New' : 'Actual'  }}</th>
                            </tr>
                          </template>
                          <template v-slot:items="props">
                            ...
                            <td>
                              <v-menu offset-y>
                                ...
                                <v-list>
                                  <v-list-tile>
                                    <v-list-tile-title @click="goToProject(props.item.id)">{{ $t('milestoneDashboard.goToProject') }}</v-list-tile-title>
                                  </v-list-tile>
                                  <v-list-tile>
                                    <v-list-tile-title @click="monthlyReview(props.item)">{{ $t('milestoneDashboard.monthlyReview') }}</v-list-tile-title>
                                  </v-list-tile>
                                  <v-list-tile>
                                    <v-list-tile-title @click="history(props.item)">{{ $t('milestoneDashboard.history') }}</v-list-tile-title>
                                  </v-list-tile>
                                </v-list>
                              </v-menu>
                            </td>
                            ...
                            <td><div :class="['boxStatus', props.item.milestoneStatus]"></div></td>
                            ...
                            <td 
                                ...
                                >
                                <!-- -->
                                <v-menu offset-y v-if="mile.type === 'new' && mile.original">
                                  <template v-slot:activator="{ on }">
                                    <v-btn
                                      :color="mile.color"
                                      flat
                                  ...
                                  <v-list>
                                    <v-list-tile>
                                      <v-list-tile-title @click="getComments(props.item, mile)">{{ $t('milestoneDashboard.showComments') }}</v-list-tile-title>
                                    </v-list-tile>
                                    <v-list-tile>
                                      <v-list-tile-title @click="selectProject(props.item, mile)">{{ $t('milestoneDashboard.changeNewForecastedDate') }}</v-list-tile-title>
                                    </v-list-tile>
                                  </v-list>
                                </v-menu>
                                <v-menu offset-y v-if="mile.type === 'actual'">
                                  <template v-slot:activator="{ on }">
                                    <v-btn
                                      flat
                                      small
                                  ...
                                  <v-list>
                                    <v-list-tile>
                                      <v-list-tile-title @click="selectActualProject(props.item, mile)">Change Actual Completion Date</v-list-tile-title>
                                    </v-list-tile>
                                  </v-list>
                                </v-menu>
      ...
      >
        <v-card>
          <v-card-title
            class="headline text-white blue lighten-2"
            primary-title
          >
            {{ $t('milestoneDashboard.comments') }}
          </v-card-title>
          <v-card-text>
            <v-list class="commentList" two-line v-if="comments.length">
            ...
            <span v-else class="noComments">{{ $t('milestoneDashboard.noComments') }}</span>
          </v-card-text>
        </v-card>
      </v-dialog>
  ...
  data() {
    return {
      headers: [
        ...
        { text: this.$t('milestoneDashboard.lastUpdated'), value: 'lastUpdateText' },
        { text: this.$t('milestoneDashboard.milestoneStatus'), value: 'MilestoneStatus' },
        { text: this.$t('milestoneDashboard.completeBudget'), value: 'completeBudget', width: '80px' },
        { text: this.$t('milestoneDashboard.completeSchedule'), value: 'completeSchedule', width: '80px' },
      ],
      milestoneHeaders: [
        { text: this.$t('milestoneDashboard.date'), value: 'date', sortable: false },
        { text: this.$t('milestoneDashboard.action'), value: 'action', sortable: false },
        { text: this.$t('milestoneDashboard.user'), value: 'user', sortable: false },
        { text: this.$t('milestoneDashboard.comments'), value: 'comment', sortable: false },
      ],
      managerFilter: [],
      campusFilter: [],
      projectStatus: [],
      ...
      milestones: ['RFP Release Date', 'RFP Due Date', 'Board Approval', 'Contruction Start', 'Go Live', 'Closeout'],
  ...
  methods: {
    ...
    selectProject(project, mile) {
      ...
      if(mile.name && mile.name.validDates) {
        this.validDates = mile.name.validDates.map(row => {
    ...
    async getDataProject(p) {
      ...
      this.milestones.forEach(row => {
        ...
        let dateOriginal = data && data.original ? this.$moment(data.original).format('MM/DD/YYYY') : ''
        let dateProjected = data && data.projected ? this.$moment(data.projected).format('MM/DD/YYYY') : ''
        let dateActual = data && data.actual ? this.$moment(data.actual).format('MM/DD/YYYY') : ''
        
        const start = this.$moment(data.original)
        const end = this.$moment(data.projected)
        const days = this.$moment.duration(start.diff(end)).asDays() * -1
        
        let color = (data.original && data.projected) ? (days <= 30 ? 'green' : days <= 90 ? 'yellow' : 'red' ): ''
  ...
  computed: {
    ...
    campus() {
      ...
      this.settings.campus.forEach(cl1 => {
        let itemName = cl1.abbr ? `(${cl1.abbr}) ${cl1.name}` : cl1.name
